...
Для обоих транспортов убедитесь, что выбран режим Bot API, а в приложении Slack включена настройка **Interactivity & Shortcuts**.
...
Для Socket Mode также проверьте, что:

- Socket Mode включён;
- app-level token начинается с `xapp-` и имеет разрешение `connections:write`;
- воркер Slack запущен и имеет исходящий доступ по HTTPS/WebSocket.
