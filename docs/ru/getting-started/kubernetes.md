...
Воркер Slack поддерживает WebSocket-соединение Socket Mode, через которое работают интерактивные кнопки `Acknowledge`, `Resolve`, `Shelve 1h` и `Unshelve`. Сами сообщения Slack отправляет веб-компонент, поэтому без этого воркера уведомления продолжат поступать — не будут работать только кнопки:
...
Для конфигурации, создаваемой чартом, до обновления добавьте необходимые
внутренние CIDR/IP:
