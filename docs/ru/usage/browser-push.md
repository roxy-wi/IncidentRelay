...
| `action_token_ttl_seconds` | Время жизни одноразовых токенов действий ACK/Resolve/Shelve/Unshelve, встроенных в push-уведомления |
...
## Кнопки ACK, Resolve и Shelve

Push-уведомления об алертах могут включать действия `Acknowledge`, `Resolve`, `Shelve 1h` и `Unshelve`. Эти кнопки используют короткоживущие одноразовые токены действий, встроенные в payload уведомления. Токены действий создаются только если назначенный пользователь в данный момент имеет responder-права на команду AlertGroup. Пользователь без responder-прав всё равно получает информационный push, но `action_tokens` остаётся пустым и кнопки действий не показываются. При использовании токена права проверяются повторно.
...
`token_expired` обычно означает, что токен действия старше `action_token_ttl_seconds`. Свежие токены **Unshelve** и **Resolve**, возвращаемые после **Shelve 1h**, действуют в течение часового shelf-периода, чтобы подтверждающее уведомление оставалось интерактивным до автоматического expiry. `token_already_used` означает, что токен действия того же уведомления уже был использован.
...
Тот же токен действия ACK/Resolve/Shelve/Unshelve уже был использован. Это может произойти после двойного клика, повторной попытки браузера или если пользователь нажал одно и то же действие уведомления более одного раза.

## Shelve

Push-уведомление может содержать **Shelve 1h**. После успешного Shelve подтверждающее уведомление получает новые одноразовые токены **Unshelve** и **Resolve** на время shelf. Из-за ограничений браузеров/ОС для firing-группы приоритет отдан Acknowledge и Shelve. При каждом действии responder-права проверяются повторно. Подробнее: [Shelving](shelving.md).
