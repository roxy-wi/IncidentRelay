...
Push-уведомления об алертах могут включать действия `Acknowledge`, `Resolve`, `Shelve 1h` и `Unshelve`. Эти кнопки используют короткоживущие одноразовые токены действий, встроенные в payload уведомления.
